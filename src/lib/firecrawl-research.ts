...
import { getFirecrawlApiKey } from "./app-config";
import { uint8ToBase64 } from "./base64";
import { firecrawlScrape, firecrawlSearch } from "./firecrawl-api";
...
export async function fetchLogoDataUrl(logoUrl: string | null): Promise<string | null> {
  ...
  try {
    ...
    if (bytes.length > 600_000) return null;
    return `data:${type.split(";")[0]};base64,${uint8ToBase64(bytes)}`;
  } catch {
    return null;
  }
}
...
async function scrapePage(
  ...
  try {
    const doc = await firecrawlScrape(apiKey, url, {
      formats: [
        "markdown",
        {
          type: "json",
...
export async function gatherCompanyResearch(
  ...
  for (const url of urlsToScrape) {
    ...
    const page = await scrapePage(apiKey, url, clientName);
