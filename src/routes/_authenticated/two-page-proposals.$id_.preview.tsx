...
export const Route = createFileRoute("/_authenticated/two-page-proposals/$id_/preview")({
  head: ({ params }) => ({ meta: [{ title: `Preview — 2-page Proposal ${params.id.slice(0, 8)}` }] }),
  component: TwoPageProposalPreviewPage,
});

function TwoPageProposalPreviewPage() {
  const { id } = Route.useParams();
  const q = useQuery({ queryKey: ["proposal", id], queryFn: () => getProposal(id) });
  const preview = q.data ? buildProposalPreview({ ...q.data, proposal_type: "two_page" }) : null;

  if (q.isLoading) return <p className="p-6">Loading preview…</p>;
  if (!preview) {
    return (
      <div className="p-6 space-y-3">
        <p>Cannot preview this proposal.</p>
        <Link to="/two-page-proposals/$id" params={{ id }} className="text-sm text-primary underline">
          Back to proposal
        </Link>
      </div>
    );
  ...
  return (
    <div className="proposal-preview-page min-h-screen bg-[#eef4f8]">
      <div className="proposal-preview-toolbar sticky top-0 z-10 flex items-center justify-between gap-3 border-b bg-background/95 backdrop-blur px-6 py-3 print:hidden">
        <Link to="/two-page-proposals/$id" params={{ id }} className="text-sm text-muted-foreground inline-flex items-center gap-1 hover:underline">
